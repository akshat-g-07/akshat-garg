...
export default function Tile({ imgSrc, alt, title, link, className }) {
  return (
    <Link
      to={link}
      className={cn(
        "border-2 border-black dark:border-white rounded-lg flex justify-center items-center transition-all duration-500 ease-in-out pointer shadow-xl hover:scale-105 group/tile-parent relative overflow-hidden aspect-video w-48 md:w-64",
        className
      )}
    >
      <img
